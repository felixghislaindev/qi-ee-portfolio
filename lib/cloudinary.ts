...
export const CLOUDINARY_CONFIG = {
  USE_CLOUDINARY: false, // Toggle this to switch between local and Cloudinary images
  ...
  FOLDER: "qi-ee-portfolio", // Cloudinary folder for organization
}
...
export function getCloudinaryUrl(
  ...
  const { width = 800, height = 1000, quality = "auto", format = "auto", crop = "fill" } = options
...
export function transformCloudinaryImage(cloudinaryResource: any): CloudinaryImage {
  return {
    ...
    year: cloudinaryResource.context?.year || new Date().getFullYear().toString(),
